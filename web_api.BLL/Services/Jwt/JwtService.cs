...
namespace web_api.BLL.Services.Jwt
{
    public class JwtService : IJwtService
    {
        ...
        public JwtService(UserManager<AppUser> userManager, IConfiguration configuration, IJwtRepository jwtRepository)
        {
        ...
        private bool ValidRefreshToken(RefreshToken token)
        {
            if (token.IsUsed)
            {
                throw new SecurityTokenValidationException("Refresh token has been used");
            }
        ...
        public async Task<ServiceResponse> RefreshTokensAsync(JwtTokensDto dto)
        {
            ...
            return new ServiceResponse("Tokens successfully refreshed", true, tokens);
        }
    }
}
